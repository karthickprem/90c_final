...
Usage:
    ...
    LIVE:      python test_90c_flow.py --live
"""
...
def get_current_window_slug() -> Tuple[str, int, int]:
    ...
    return f"sol-updown-15m-{start}", start, end
...
def get_next_window_slug() -> Tuple[str, int, int]:
    ...
    return f"sol-updown-15m-{next_start}", next_start, next_end
...
async def test_90c_flow_multiwindow():
    ...
    log_filename = f"90c_sol_test_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
    logger = DualLogger(log_filename)
    
    # Excel file for trade data
    excel_filename = f"90c_sol_trades_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
...
if __name__ == "__main__":
    ...
    print("="*80)
    print(f"\n  Usage:")
    print(f"    Dry run:  python test_90c_flow.py")
    print(f"    Live:     python test_90c_flow.py --live")
    print()
